...
export class TaskService {
    ...
    async createTask(data:TaskCreateDTO){
        ...
        const user = await this.taskModel.checkUser(data);
        if(!user) throw new Error("User not found");

        if (data.priorityId){
            const priority = await this.taskModel.checkPriority(data.priorityId);
    ...
    async getAllTasks(userId:string){
        ...
        return await this.taskModel.getAllTasks(userId);     
    }

    // get task by id 
    async getTaskById(taskId:string,userId:string){
        const task = await this.taskModel.getTaskById(taskId,userId);
    ...
    async updateTask(taskId:string,userId:string, updates:TaskUpdateDTO){
        ...
        const updatedTask = await this.taskModel.updateTask(taskId,userId,updates);
        return updatedTask;
    }

    // delete task
    async deleteTask(taskId:string){
        if(!taskId) throw new Error ("Task id is not found");
